...
export function TransactionForm(props) {
  ...
  useEffect(() => {
    if (transaction.address.length == 42) {
      fetchContractAbi(transaction.address)
        .then((res) => {
    ...
    <Paper elevation={2}>
      <Stack spacing={2} p={2}>
        ...
        >
          ...
          <IconButton onClick={() => removeTransaction(index)} color="error">
            <DeleteIcon />
          </IconButton>
        </Stack>
        <TextField
                ...
                action.inputs.map((input, i) => {
                  return (
                    <ParamInput
                      ...
                      label={input.name}
                      type={input.type}
                      value={transaction.params[1][i]}
                      setValue={(val) => handleParamChange(i, val)}
                    />
                  );
                })}
              {action?.payable && (
              ...
              {transaction.params[0].map((type, i) => {
                return (
                  <ParamInput
                    ...
                    type={type}
                    value={transaction.params[1][i]}
                    setType={(val) => handleTypeChange(i, val)}
                    setValue={(val) => handleParamChange(i, val)}
                  />
                );
              })}
