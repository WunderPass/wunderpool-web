...
export default function CustomForm(props) {
  ...
  const addTransaction = () => {
    setProposalTransactions([...proposalTransactions, {address: "", action: "", params: [[], []], value: ""}]);
  }
  ...
  const handleSubmit = (e) => {
    ...
    createCustomProposal(poolAddress, proposalTitle, proposalDescription, proposalContracts, proposalActions, proposalParams, proposalValues, Math.round(new Date(proposalDeadline).getTime() / 1000)).then((res) => {
      console.log(res);
  ...
  return (
    <form>
      <Stack width="100%" spacing={3}>
        ...
        <TextField label="Title" required value={proposalTitle} onChange={(e) => setProposalTitle(e.target.value)} placeholder="Buy a WunderPass NFT"/>
        <TextField label="Description" value={proposalDescription} onChange={(e) => setProposalDescription(e.target.value)} placeholder="We will buy a WunderPass NFT for 420 MATIC"/>
        <FormControl>
          <InputLabel>Deadline</InputLabel>
          <OutlinedInput type="datetime-local" label="Deadline" value={proposalDeadline} onChange={(e) => setProposalDeadline(e.target.value)} inputProps={{min: new Date().toISOString().slice(0,-5)}}/>
        </FormControl>

        {proposalTransactions.map((transaction, i) => {
          return (
            <TransactionForm key={`transaction-${i}`} index={i} transaction={transaction} updateTransaction={(property, val) => updateTransaction(i, property, val)} removeTransaction={removeTransaction} {...props}/>
          )
        })}
        <Button startIcon={<AddIcon />} onClick={addTransaction}>Add Transaction</Button>
        {loading ? 
          <Button disabled variant="contained">Submitting Proposal...</Button> :
          <Button type="submit" disabled={false} onClick={handleSubmit} variant="contained">Submit Proposal</Button>
        }
      </Stack>
    </form>
  )
