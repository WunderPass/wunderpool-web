...
export default function SellTokenDialog(props) {
  ...
  const handleSubmit = () => {
    setLoading(true);
    createFudSuggestion(poolAddress, address, `Let's Sell ${name} (${symbol})`, `We will sell ${amount} ${symbol}`, amount).then((res) => {
      console.log(res);
    ...
    <Dialog open={open} onClose={handleClose}>
      ...
      {loading ? 
        ...
        <DialogActions>
          ...
          <Button onClick={handleSubmit} color="success" disabled={Number(amount) > Number(balance)}>Sell</Button>
        </DialogActions>
      }
    </Dialog>
  )
