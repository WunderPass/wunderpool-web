export default function useWunderPass(config) {
  ...
  const smartContractTransaction = (tx, usdc={}, network="polygon") => {
    return new Promise(async (resolve, reject) => {
      try {
        const popup = window.open(encodeURI(`https://app.wunderpass.org/smartContract?name=${name}&imageUrl=${image}`), 'WunderPassContract', 'tab');
        
        const requestInterval = setInterval(() => {
          popup.postMessage(JSON.parse(JSON.stringify({accountId: accountId, tx: tx, network: network, usdc: usdc})), 'https://app.wunderpass.org')
        }, 1000);
