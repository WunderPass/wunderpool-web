...
export default function AdminRewardsPage(props) {
  ...
  const fetchUserData = async () => {
    setLoading(true);
    const { data } = await axios({ url: '/api/users/all' });
    setLoading(false);
    setAllUsers(data);
  };
      ...
      allUsers.forEach(async (usr) => {
        const usdBalance =
          ...
          (await cacheItemDB(
            ...
            await usdcBalanceOf(usr.wallet_address),
            600
          ));
...
function UserRow({ user, handleSuccess }) {
  const [loading, setLoading] = useState(false);
...
function CopyEmailsButton({ userData, handleSuccess }) {
