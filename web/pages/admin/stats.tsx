...
function ActiveUserRow({
  ...
  const [userData, setUserData] = useState<FormattedUser>(null);
...
function MembersPerGameTooltip({
  active,
  payload,
  label,
...
function GamesPerMemberTooltip({
  active,
  payload,
  label,
}: TooltipProps<ValueType, NameType>) {
...
export default function AdminStatsPage(props: AdminStatsPageProps) {
  ...
  const compareData = useMemo(() => {
    const lookupDate = new Date(Number(new Date()) - timeFrame * 86400000);
