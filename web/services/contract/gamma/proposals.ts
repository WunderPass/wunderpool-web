import { TransactionData, SupportedChain, FormattedProposal } from './../types';
...
export async function fetchPoolProposalsGamma(address, userAddress = null) {
  try {
...
export function fetchTransactionDataGamma(
  address,
  id,
  transactionCount
...
export function executeProposalGamma(
  poolAddress,
  id,
  version,
  ...
  return new Promise(async (resolve, reject) => {
    ...
    smartContractTransaction(tx, null, chain, popup)
      .then(async (transaction) => {
        try {
...
export function isLiquidateProposalGamma(poolAddress, id): Promise<boolean> {
  return new Promise(async (resolve, reject) => {
    try {
      const [wunderPool] = initPoolGamma(poolAddress);
