...
import { SupportedChain, FormattedProposal } from './types';
...
export function fetchPoolProposals(
  address: string,
  userAddress: string,
  version: number
): Promise<FormattedProposal[]> {
  if (version > 4) {
    return new Promise(async (resolve, reject) => {
      axios({ url: `/api/proposals?address=${address}` })
        .then(({ data }: { data: ProposalsResponse }) => {
          resolve(
...
export function fetchTransactionData(
  ...
  if (version > 5) {
    return fetchTransactionDataEta(address, id, transactionCount, chain);
  } else if (version > 5) {
    ...
    sendSignatureRequest(types, values, false, popup)
      .then((sig) => {
        axios({
          ...
          .then((res) => {
            waitForTransaction(res.data, options.chain)
              .then((tx) => {
                resolve(tx);
              })
              .catch((err) => {
...
export function proposalExecutable(
  poolAddress: string,
  id: number,
  version: number
) {
...
export function executeProposal(
  ...
  return executeProposalGamma(poolAddress, id, version, chain);
}

export function isLiquidateProposal(
  poolAddress: string,
  id: number,
  version: number,
  chain: SupportedChain
  ...
  if (version > 6) {
    return isLiquidateProposalEta(poolAddress, id, chain);
  } else if (version > 5) {
